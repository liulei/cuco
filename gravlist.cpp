...
/*	some GPU work:	*/
	...
	numGridCells	=	PMGRID * PMGRID * PMGRID;
	...
	allocateArray((void **) &dOldPos, memSize);
	allocateArray((void **) &dSortedPos, memSize);
	allocateArray((void **) &dGravAccel, memSize);
	...
	sorter	=	new RadixSort(numParticles);
...
void gravity_linklist_cuda(){

	printf("set softening...\n");
	set_softenings();
	setSoftParam(&hSoftParam);

	printf("copy position to device...\n");
	copyPosToDevice();

	printf("calculate hash...\n");
	calcHash(dGridParticleHash, 
			 dGridParticleIndex,
			 dOldPos,
			 numParticles);

	printf("sorting...\n");
	sorter->sort(dGridParticleHash, 
				 dGridParticleIndex,
				 numParticles,
				 18);
	...
	printf("shortrange force...\n");
	cudaForceEvaluateShortrange(
			dGravAccel,
			dSortedPos,
			dGridParticleIndex,
			dCellStart,
